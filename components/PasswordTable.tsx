...
import { Eye, EyeOff, Edit, Trash, LogOut, Plus, Search, X, CheckCircle, AlertCircle, Info } from 'lucide-react';
...
export default function PasswordTable() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setEditingId(null);
        fetchCredentials();
        showAlert('success', editingId ? 'Credential updated successfully' : 'Credential added successfully');
      } else {
        setError('Failed to save credential');
        showAlert('error', 'Failed to save credential');
      }
    } catch (err) {
      setError('An error occurred');
      showAlert('error', 'An error occurred while saving credential');
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        fetchCredentials();
        showAlert('success', 'Credential deleted successfully');
      } else {
        setError('Failed to delete credential');
        showAlert('error', 'Failed to delete credential');
      }
    } catch (err) {
      setError('An error occurred');
      showAlert('error', 'An error occurred while deleting credential');
    }
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Alert Banner */}
      {alert.type && (
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md rounded-md shadow-lg ${
          ...
          <div className="flex items-center">
            {alert.type === 'success' && <CheckCircle className="h-5 w-5 text-green-500 mr-2" />}
            {alert.type === 'error' && <AlertCircle className="h-5 w-5 text-red-500 mr-2" />}
            {alert.type === 'info' && <Info className="h-5 w-5 text-blue-500 mr-2" />}
            <p className={`text-sm ${
